...
class Board:
    ...
    def get_reward_by_action(self, x, y, i):
        ...
        # 自转惩罚
        if self.Punishment[next_x][next_y] != self.prize_value and x == next_x and y == next_y:
    ...
    def show(self, text=None, show_chessboard=True):
        """
        可视化
        :param text: 文本
        :param show_chessboard: 是否选择展示地图
        :return:None
        """
        # 更新策略
        self.update_policy()
    ...
    def get_optimal_action(self, x, y):
        ...
        for i in range(self.sum_action):
            if self.state_action_probabilities[x][y][i] > self.state_action_probabilities[x][y][max_index]:
                max_index = i
        return max_index
